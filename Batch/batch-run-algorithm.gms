...
$ONTEXT
    ...
    This script solves an instance of the mixed AC-DC model by calling
    'solve-acdc-model.gms' with the appropriate parameters. The solution
    algorithm may be run with or without bound tightening (or both ways) by
    setting the flags %RunWithBT% and %RunWithoutBT%, respectively. If both
    flags are set, the script will run the bound tightened version first.

    The user can also explicitly turn off running Big M computation or bound
    tightening. This is useful when, for instance, repeating the algorithm part
    only with different settings.
    
    This script is intended to be edited and adapted by the user.

    NOTES:
    1. The linear relaxation is hard-coded for simplicity, since it gives the
       best results.
    2. This particular version of the script includes default options files for
       CPLEX, GUROBI, BARON, GLoMIQO, and LINDOGlobal. Option file generation
       for unused solvers may be safely removed.
    3. For more insight on each step of the solution algorithm, see the notes
       within the test scripts in the 'Test' folder.
$OFFTEXT
...
** GUROBI
*   - Use barrier method at root node
*   - Tailored cuts (NOTE: -1 = auto, 0 = off)
$ONECHO > gurobi.op2
method 2
zerohalfcuts 0
cliquecuts -1
